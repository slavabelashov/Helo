...
int ThreadTicks = 15;
int CurContextId = 0;
int ThreadOneSleepTime = 10;
int ThreadTwoSleepTime = 8;
...
void thread_exit()
{
	MyTreads[CurContextId].ContextStatus = 0;
}
...
{
	MyTreads[CurContextId].ContextStatus = 2;
	MyTreads[CurContextId].ContextSleepTime = counts;
	int TempContextId = CurContextId;
	CurContextId = 0;
...
void thread_1()
{
	...
	for(i=0; i<ThreadTicks; i++)
	{
		...
		thread_sleep(ThreadOneSleepTime);
	}
}
...
void thread_2()
{
	...
	for (i=0; i<ThreadTicks; i++)
	{
		...
		thread_sleep(ThreadTwoSleepTime);
	}
}
...
void swap_threads(int tid)
{
	...
	{
		CurContextId = tid;
		swapcontext(&SchedulerContext, &MyTreads[CurContextId].Context);
	}
}
...
void schedul_function()
{
	...
	while(CurrentId < NumberOfContexts)
	{
		if ((CurContextId!=CurrentId)&&(MyTreads[CurrentId].ContextStatus==1))
		{
			NextId = CurrentId;
			break;
		}
